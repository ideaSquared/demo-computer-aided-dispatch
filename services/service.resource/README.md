# @cad/service.resource

> **One-liner:** Unit roster, status, last-known location.
...
[**PRD — service.resource**](https://www.notion.so/37389ffb19fc81e48b4cdcc43f19cdcc)
...
```bash
pnpm --filter @cad/service.resource build
```

## Test

```bash
pnpm --filter @cad/service.resource test
pnpm --filter @cad/service.resource typecheck
```
...
- This service owns Postgres schema `resource`. No cross-schema joins; talk to other services via gRPC or events.
