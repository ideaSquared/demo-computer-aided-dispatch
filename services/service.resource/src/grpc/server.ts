import { HealthV1, ResourceV1 } from '@cad/proto';
import * as grpc from '@grpc/grpc-js';

/**
 * Build and start the gRPC server. We attach two services:
 *
 *   - ResourceService — the business surface.
...
export async function startGrpcServer(opts: {
  port: number;
  handlers: ResourceV1.ResourceServiceServer;
  log: { info: (o: unknown, m?: string) => void };
}): Promise<grpc.Server> {
  const server = new grpc.Server();
  server.addService(ResourceV1.ResourceServiceService, opts.handlers);
  ...
  opts.log.info({ port: boundPort }, 'gRPC ResourceService listening');
  return server;
}
