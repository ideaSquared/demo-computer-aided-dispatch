import { HealthV1, IncidentV1 } from '@cad/proto';
import * as grpc from '@grpc/grpc-js';

/**
 * Build and start the gRPC server. We attach two services:
 *
 *   - IncidentService — the business surface.
...
export async function startGrpcServer(opts: {
  port: number;
  handlers: IncidentV1.IncidentServiceServer;
  log: { info: (o: unknown, m?: string) => void };
}): Promise<grpc.Server> {
  const server = new grpc.Server();
  server.addService(IncidentV1.IncidentServiceService, opts.handlers);
  ...
  opts.log.info({ port: boundPort }, 'gRPC IncidentService listening');
  return server;
}
