...
> **One-liner:** Append-only audit log consumer.
...
pnpm --filter @cad/service.audit dev         # watch mode (tsx), port 5070
curl http://localhost:5070/health           # → { "status": "ok" }
```
...
- This service owns Postgres schema `audit`. No cross-schema joins; talk to other services via gRPC or events.
