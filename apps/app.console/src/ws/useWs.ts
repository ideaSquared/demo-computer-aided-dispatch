...
export interface UseWsOptions {
  /**
   * URL relative to the current origin (Vite/edge proxy forwards `/ws` to the
   * gateway), or absolute `ws(s)://` for direct connects. Query params carry
   * the Phase-1 identity stub.
   */
  url: string;
  /** Receives every server event for a topic the caller subscribed to. */
  onEvent?: (topic: string, payload: unknown) => void;
  /** Receives error frames `{type:'error',code,message?}` from the server. */
 ...
 *
 * Lifecycle: one socket per hook instance per URL. Re-renders that change
 * `url` reconnect; changing `onEvent` / `onError` does not (callbacks are
 * stashed in refs so each render's closure stays current without churning
 * the connection).
 */
export function useWs(opts: UseWsOptions): {
  ...
  // Refs survive reconnects so subscriptions resume automatically.
  ...
  useEffect(() => {
    ...
    function connect(): void {
      ...
      socket.addEventListener('open', () => {
        ...
        setStatus('open');
        // Resume every topic with a live listener — the gateway's per-pod
        // registry was cleared on disconnect, so we replay.
